...
ordenadas $D^1_i<D^2_i<\dots<D^{K_i}_i$. Se introduce $z^k_i=1$ si y solo si el cliente $i$
\emph{aún no está cubierto} al radio $D^k_i$ (no hay sitio abierto a distancia $\le D^k_i$).
...
$z^k_i$ por cada \emph{radio distinto} $D^k_i$, y los radios distintos $K_i$ suelen ser muchos
menos que $M$. Esa es la razón estructural por la que F2 nunca es mayor que F1 y normalmente es
mucho menor, manteniendo la misma cota LP.

\begin{align}
  \min\ & \sum_{i=1}^{N}\Big[\,D^1_i + \sum_{k=1}^{K_i-1}\big(D^{k+1}_i-D^k_i\big)z^k_i\,\Big] \label{eq:f2obj}\\
  \text{s.a.}\ & \sum_{j=1}^{M} y_j=p \nonumber\\
  & z^k_i + \!\!\sum_{j:\,d_{ij}\le D^k_i}\!\! y_j \ge 1 && i\in[N],\,k\in[K_i] \label{eq:f2cov}\\
  & z^k_i\ge 0,\quad y_j\in\{0,1\}. \nonumber
\end{align}
...
F2 tiene $K=\sum_i K_i$ variables $z$ (con $K\le N\times M$, usualmente $K\ll N\times M$) y
...
\begin{align}
  ...
  & z^k_i + \!\!\sum_{j:\,d_{ij}=D^k_i}\!\! y_j \ge z^{k-1}_i && i\in[N],\,k=2,\dots,K_i \label{eq:f3rec}\\
  & z^k_i\ge 0,\quad y_j\in\{0,1\}. \nonumber
\end{align}
...
\eqref{eq:f4cut} sobre la marcha} (perezosamente) dentro del branch-and-cut.
...
\bottomrule
\end{tabular}
\caption{$K=\sum_i K_i\le N\times M$. Las cuatro tienen el mismo óptimo entero y la misma cota LP;
