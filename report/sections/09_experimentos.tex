...
para el comparativo), \texttt{MIPGap}$=10^{-10}$, sin límite de tiempo (todas las instancias
cierran a optimalidad). \textbf{Sin} PopStar (arranque uniforme + redondeo), sin \emph{reduced-cost
fixing} ni \emph{constraint reduction}. Todos los números provienen de
\texttt{results/} (CSV y \texttt{logs/}); cada corrida deja un log con prueba de callback.

Se reportan tres bloques: (i) corrección y desempeño en OR-Library; (ii) comparación contra el
paper en \texttt{rl1304}; (iii) efecto del warm-start. Cuatro figuras agregan el comportamiento.
...
Fuente: \texttt{results/orlib\_optima\_check.csv}.
...
Sobre las mismas instancias TSPLIB se comparan los óptimos y cotas (los valores del paper son
transcritos del PDF). Fuente: \texttt{results/comparison\_vs\_paper.csv}.
...
\texttt{results/warmstart\_comparison.csv}.

\begin{table}[h]\centering\small
\begin{tabular}{lrr rr rr}
\toprule
 & \multicolumn{2}{c}{nodos B\&B} & \multicolumn{2}{c}{cortes lazy} & \multicolumn{2}{c}{$T_2$ [s]} \\
...
\bottomrule
\end{tabular}
\caption{Warm vs cold. Los nodos y los cortes lazy caen fuertemente. El wall-time es
...
\texttt{lazy\_cuts=1850}. Cortes lazy $>0$ confirma que la Fase 2 \emph{no} degenera a un B\&B
ordinario.

\subsection{Figuras agregadas}
Las cuatro figuras (generadas solo de CSV reales por \texttt{scripts/plot\_results.py}) son:
\begin{figure}[h]\centering
\includegraphics[width=0.48\textwidth]{../results/plot_a_bounds_orlib.png}\hfill
\includegraphics[width=0.48\textwidth]{../results/plot_b_time_vs_N.png}
\caption{(izq.) $LB_1$, $UB_1$ y óptimo por instancia OR-Library. (der.) Tiempo total vs $N$ (log-log).}
\end{figure}
\begin{figure}[h]\centering
\includegraphics[width=0.48\textwidth]{../results/plot_c_gap_vs_pM.png}\hfill
\includegraphics[width=0.48\textwidth]{../results/plot_d_iter_nodes_vs_p.png}
