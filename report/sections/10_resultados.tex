...
\begin{enumerate}[leftmargin=1.5em]
  ...
  \item \textbf{El método es correcto a escala creciente.} 15/15 óptimos en OR-Library y 9/9 en
        \texttt{rl1304} ($N=1304$, hasta $p=500$) con $\Delta=0$ respecto del paper
...
\begin{itemize}[noitemsep]
  ...
  \item \textbf{Sin comparación directa con Zebra:} no se reejecutó; los tiempos del paper son
        ajenos a nuestra máquina.
...
\begin{itemize}[noitemsep]
  \item \textbf{La formulación importa más que el solver.} F1--F4 tienen el mismo óptimo y la misma
        cota LP, pero su estructura (densidad, número de variables) decide qué es resoluble. La
        ganancia vino de \emph{explotar estructura}, no de fuerza bruta del solver.
...
eficiente para el pMP. El método alcanza el óptimo en todas las instancias probadas (OR-Library
15/15, \texttt{rl1304} 9/9 vs paper), con un separador verificado por tres vías independientes.
El trabajo cumple los objetivos del curso: formulación rigurosa, propuesta lagrangiana, diseño y
