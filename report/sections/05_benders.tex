...
\begin{align}
  ...
  & z^k_i - z^{k-1}_i \ge -\!\!\sum_{j:\,d_{ij}=D^k_i}\!\!\bar y_j && k=2,\dots,K_i-1 \label{eq:spk}\\
  & z^k_i\ge 0 && k=1,\dots,K_i-1. \nonumber
\end{align}
...
Con variables duales $v^k_i\ge0$ para $k=1,\dots,K_i-1$ (una por restricción primal) y la
convención $v_i^{K_i}=0$:
\begin{align}
  \max\ & D^1_i + v^1_i\Big(1-\!\!\sum_{j:\,d_{ij}=D^1_i}\!\!\bar y_j\Big) - \sum_{k=2}^{K_i-1} v^k_i\!\!\sum_{j:\,d_{ij}=D^k_i}\!\!\bar y_j \nonumber\\
  \text{s.a.}\ & v^k_i - v^{k+1}_i \le D^{k+1}_i-D^k_i && k=1,\dots,K_i-1 \label{eq:dspcon}\\
  & v^k_i\ge 0 && k=1,\dots,K_i-1. \nonumber
\end{align}
...
\begin{equation}
  ...
  - \sum_{k=2}^{K_i-1}\bar v^k_i\!\!\sum_{j:\,d_{ij}=D^k_i}\!\! y_j.
  \label{eq:cut16}
\end{equation}
...
La razón es estructural. En pMP siempre se abren exactamente $p\ge1$ sitios y todo cliente puede
asignarse a alguno de ellos; no existen restricciones de capacidad ni asignaciones prohibidas en
el modelo implementado. Incluso si el maestro propone un vector $\bar y$ fraccional durante la
relajación, el subproblema F3 del cliente conserva variables $z\ge0$ que pueden tomar valores
suficientes para satisfacer la cadena de restricciones. Por eso el subproblema nunca ``falla''; a
lo sumo revela que $\bar\theta_i$ estaba subestimando la distancia, y eso se corrige con un corte
de optimalidad.
...
lazy constraints termina con el óptimo exacto.
...
árbol de branch-and-bound mediante un callback de lazy constraints: esto es
