...
\textbf{Dualidad débil.} Para todo $\lambda$, $L(\lambda)\le \OPT(pMP)$ —en la notación del curso,
$\theta(\lambda)\le z^\star$, escribiendo $\theta(\lambda)$ para la función dual lagrangiana—: toda
elección de multiplicadores entrega una \emph{cota inferior} válida. El \textbf{dual lagrangiano}
busca la mejor:
\[
  \max_{\lambda}\ L(\lambda).
\]
$L(\lambda)$ es \emph{cóncava y lineal por tramos} (mínimo puntual de funciones afines en
$\lambda$), por lo que en general es \emph{no diferenciable} y se maximiza con el \textbf{método
del subgradiente}. Dado $\lambda^t$ se resuelve el subproblema~\eqref{eq:lagsub} (barato), se
obtiene el subgradiente $g^t_i = 1-\sum_j x^t_{ij}$ (la violación de la restricción de asignación
del cliente $i$) y se actualiza con \textbf{paso de Polyak}:
\[
  \lambda^{t+1}_i=\lambda^t_i+\alpha_t\,g^t_i,
  \qquad
  \alpha_t=\beta_t\,\frac{UB-L(\lambda^t)}{\lVert g^t\rVert^2},
  \quad 0<\beta_t<2,
\]
donde $UB$ es la mejor cota superior primal conocida. Se mantiene en todo momento
$LB\le z^\star\le UB$ y se mide el gap $\frac{UB-LB}{|UB|}$.
...
el subproblema de Benders resulta resoluble \emph{en forma cerrada}, lo que lo hace
particularmente ventajoso aquí; pero conceptualmente la lagrangiana es la alternativa natural
que el curso pide analizar.
