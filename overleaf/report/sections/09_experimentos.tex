...
paper en \texttt{rl1304}; (iii) baseline monolítico F1 parcial; (iv) grilla sintética de estrés;
...
fue una única campaña monolítica: son corridas controladas y trazadas en CSV/logs distintos, todas
...
\subsection{Baseline monolítico F1 parcial}
La pauta del curso pide una implementación base. Se implementó y ejecutó un baseline monolítico
F1 solo en cinco instancias pequeñas/medianas; esto \textbf{no} es un benchmark completo de
F1--F4. Evidencia: \texttt{report/evidence/csv/monolithic\_f1\_300s.csv}.
...
no se implementaron F2/F3/F4 monolíticas como campaña completa.}
...
\toprule
 & \multicolumn{2}{c}{nodos B\&B} & \multicolumn{2}{c}{lazy cuts} & \multicolumn{2}{c}{$T_2$ [s]} \\
...
\bottomrule
\end{tabular}
\caption{Warm-start vs cold-start. Los nodos y los lazy cuts caen fuertemente. El wall-time es
