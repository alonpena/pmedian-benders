...
La formulación ejecutable del repositorio es \textbf{Benders/F4 con lazy constraints derivada de
F3}. Esto no significa que se cargue F4 completa al solver: F4 completa sería una formulación
estándar cargada de una vez (\emph{monolithic model}) y sería densa. Aquí se usa el maestro de
Benders con variables $y_j$ y $\theta_i$, y los cortes de tipo F4 se agregan bajo demanda como
\emph{lazy constraints}. En el informe se presentan F1, F2, F3 y F4 como linaje matemático, pero
el solver C no expone formulaciones estándar completas seleccionables para F1/F2/F3/F4. En
particular, no se materializan variables $x_{ij}$ de F1 ni variables $z_i^k$ de F2/F3 en el
maestro C. La implementación Python de referencia y la fuerza bruta se usan para validación en
instancias pequeñas, no como benchmark principal.
...
\begin{description}[leftmargin=1.5em]
  ...
  \item[Fase 2 — branch-and-Benders-cut.] Se imponen $y_j\in\{0,1\}$ y se resuelve con
        ...
        \textbf{callback de lazy constraints} ejecuta el separador y agrega los cortes
...
una implementación Python de referencia. La implementación es \emph{evidencia} de que la teoría funciona, no la
...
Verificación por fuerza bruta (instancias pequeñas) & \texttt{src/instance.c} & \texttt{instance\_brute\_force} \\
...
\midrule
Implementación de referencia de correctitud & \texttt{prototype/pmp\_benders.py} & \texttt{separate\_client}, \texttt{phase2} \\
...
callback de lazy constraints. Esto deja el núcleo (parsing, distancias, $S$, separación, fases)
